...
def clean_and_map_classes(match):
    ...
    # First, remove ALL existing dark: classes to reset state
...
for root, dirs, files in os.walk('D:/PersonalOS'):
    for file in files:
        if file.endswith('.html'):
            ...
            # Use regex to find and replace class contents
            content = re.sub(r'class="([^"]+)"', clean_and_map_classes, content)
            
            # Remove any duplicate whitespace in classes just to keep it clean
