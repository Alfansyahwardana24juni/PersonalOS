...
fs.writeFileSync('D:/PersonalOS/js/inbox_app.js', inboxAppCode);

// 2. Modify inbox.html
let inboxHtml = fs.readFileSync('D:/PersonalOS/inbox.html', 'utf8');

const newMainContent = `
<!-- Main Content --> 
<div class="flex-1 flex flex-col bg-gray-50/50 dark:bg-black/20">
    <!-- Input Area -->
    <div class="px-6 py-4 border-b border-border bg-white dark:bg-surface dark:border-[#27272a] shrink-0 shadow-sm z-10">
        <div class="max-w-4xl mx-auto flex gap-3">
            <textarea id="inbox-input" rows="2" class="flex-1 block w-full p-3 bg-gray-50 border border-border rounded-xl text-sm focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand shadow-inner transition-all dark:bg-[#09090b] dark:border-[#27272a] dark:text-white resize-none custom-scrollbar" placeholder="Ketik apa saja yang ada di pikiran Anda..." onkeydown="if(event.key==='Enter' && !event.shiftKey) { event.preventDefault(); addInboxItem(); }"></textarea>
            <button onclick="addInboxItem()" class="px-5 py-2 bg-brand text-white font-medium rounded-xl hover:bg-brand/90 transition-colors shadow-sm self-end">
                Simpan
            </button>
        </div>
        <p class="text-[10px] text-gray-400 max-w-4xl mx-auto mt-2 text-right">Tekan <kbd class="px-1 bg-gray-100 rounded dark:bg-[#27272a]">Enter</kbd> untuk menyimpan, <kbd class="px-1 bg-gray-100 rounded dark:bg-[#27272a]">Shift+Enter</kbd> untuk baris baru.</p>
    </div>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
        <div class="max-w-4xl mx-auto w-full">
            <div class="flex items-center justify-between mb-6"> 
                <h2 class="text-lg font-bold text-primary">Kotak Masuk (<span id="inbox-count">0</span>)</h2> 
            </div> 
            <div id="inbox-container" class="space-y-4">
                <!-- Inbox items will be rendered here -->
            </div>
        </div>
        <!-- Bottom spacing for mobile navigation --> <div class="h-24 md:h-10"></div>
    </div>
</div>
`;

// Replace everything between <!-- Main Content --> and <!-- Mobile Bottom Navigation
inboxHtml = inboxHtml.replace(/<!-- Main Content -->[\s\S]*?<!-- Mobile Bottom Navigation/s, newMainContent + '\n<!-- Mobile Bottom Navigation');

// Replace dummy script
inboxHtml = inboxHtml.replace(/<script>\s*function convertToTask[\s\S]*?<\/script>/, '');

// Ensure dexie and db.js and inbox_app.js are included
if (!inboxHtml.includes('inbox_app.js')) {
    inboxHtml = inboxHtml.replace(/<\/body>/, '    <script src="https://unpkg.com/dexie/dist/dexie.js"></script>\n    <script src="js/db.js"></script>\n    <script src="js/inbox_app.js"></script>\n</body>');
}

fs.writeFileSync('D:/PersonalOS/inbox.html', inboxHtml);
console.log('Modified inbox.html');
